...
try {
  ...
  data.verified = '10 Sep 2026 — battlegroup recommendation UI cleanup. The visual battlegroup tree is shown before the numbered pick order, while explanatory recommendation notes, scope/confidence chips, branch/tier annotations, the Visual battlegroup tree heading/intro, and the Legal/Recommended legend remain removed. Local icons, highlighted recommended nodes, and yellow pick-order badges remain unchanged.';
  await fs.writeFile(JSON_PATH, JSON.stringify(data, null, 2));
} catch (err) {
  console.warn('Could not update JSON verified metadata:', err.message);
}

console.log('Cleaned battlegroup recommendation UI and moved the visual tree above the numbered pick order.');
