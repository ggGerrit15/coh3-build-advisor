...
function cohdbStatsSection(p) {
  ...
  return `<div class="card"><h3>Imported CoHDB statistics</h3><div class="statline">${bgStat ? `<div class="statpill"><div class="statnum">${bgStat.win_rate.toFixed(1)}%</div><div class="small muted">Battlegroup all modes</div><div class="small muted">n=${bgStat.selections.toLocaleString()}</div></div>` : ''}${modeStat ? `<div class="statpill"><div class="statnum">${modeStat.win_rate.toFixed(1)}%</div><div class="small muted">${p.mode} archetype</div><div class="small muted">n=${modeStat.games.toLocaleString()}</div></div>` : ''}${ratingStat ? `<div class="statpill"><div class="statnum">${ratingStat.win_rate.toFixed(1)}%</div><div class="small muted">${elo.value} archetype</div><div class="small muted">n=${ratingStat.games.toLocaleString()}</div></div>` : ''}</div>${source ? `<p class="small muted" style="margin-top:9px"><strong>Matched CoHDB archetype:</strong> ${escapeHtml(sourceLabel)}${a.hits ? ` · ${a.hits} matching core steps` : ''}. This is faction-level build evidence, not a battlegroup-specific win rate.</p><p class="small muted">The imported statistics are evidence only; the recommendation remains based on the curated profile, matchup logic and technical QA.</p><a href="${source.source_url}" target="_blank" rel="noopener noreferrer">Open imported CoHDB build ↗</a>` : ''}</div>`;
}
