...
const recommendationAnchor = '${smartSection()}<div class="layout">';
const recommendationReplacement = '${smartSection()}${advisorSectionHeader("Recommendation", "The displayed build is the curated, technically checked plan. Smart ranking helps choose the battlegroup; it does not replace the stored build.")}<div class="layout">';
if (!html.includes(recommendationReplacement)) {
  if (!html.includes(recommendationAnchor)) throw new Error('Could not find recommendation section insertion point');
  html = html.replace(recommendationAnchor, recommendationReplacement);
}

const statisticsAnchor = '${variantSection(p)}${eloPanel(p)}${factionStatsSection(p)}${cohdbStatsSection(p)}<div class="card"><h3>';
const statisticsReplacement = '${variantSection(p)}${advisorSectionHeader("Statistics", "Imported CoHDB records and archetype context are shown separately from the recommendation logic.")}${eloPanel(p)}${factionStatsSection(p)}${cohdbStatsSection(p)}<div class="card"><h3>';
if (!html.includes(statisticsReplacement)) {
  if (!html.includes(statisticsAnchor)) throw new Error('Could not find statistics section insertion point');
  html = html.replace(statisticsAnchor, statisticsReplacement);
}
